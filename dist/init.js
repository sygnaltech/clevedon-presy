...
function initEngine() {
    ...
    const engineModeCommand = (0, util_1.getQueryParam)('engine.mode');
    ...
    switch (engineMode) {
        ...
        case 'prod':
        default:
            const scriptUrl = (0, util_1.getCurrentScriptUrl)();
            if (scriptUrl) {
                const engineScriptUrl = scriptUrl.replace('init.js', 'index.js');
                (0, util_1.loadScript)(engineScriptUrl);
                break;
            }
    }
}
...
function invokeDebugMode() {
    ...
    links.forEach(link => {
        ...
        if (devHref) {
            (0, util_1.replaceCSSLink)(link, devHref);
        }
    });
